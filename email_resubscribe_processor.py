...
def is_resubscribe_request(content):
    ...
    # First check if it's an unsubscribe request (should be handled by unsubscribe processor)
    unsubscribe_keywords = ['unsubscribe', 'stop emails', 'remove me', 'opt out']
    for keyword in unsubscribe_keywords:
        if keyword in content_lower:
            return False
    
    # Now check for resubscribe keywords
    ...
    # Also check for standalone "subscribe" but not if preceded by "un"
    if 'subscribe' in content_lower and 'unsubscribe' not in content_lower:
        resubscribe_keywords.append('subscribe')
